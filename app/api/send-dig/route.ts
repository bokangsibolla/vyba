...
function buildEmailHtml(name: string, playlists: PlaylistSection[]): string {
  ...
  const sections = playlists.map((pl) => {
    ...
    const trackPreview = pl.tracks.slice(0, 4).map((t, i) =>
      `<tr>
        <td style="padding:4px 8px 4px 0;font-family:Courier,monospace;font-size:11px;color:#bbb;">${String(i + 1).padStart(2, '0')}</td>
    ...
    return `
      <div style="border:2px solid #E5DDD0;border-radius:12px;overflow:hidden;margin-bottom:16px;">
        <div style="background:${colors.bg};padding:10px 16px;display:flex;justify-content:space-between;">
        ...
        <div style="padding:12px 16px;">
          <table cellpadding="0" cellspacing="0" border="0" width="100%">${trackPreview}</table>
          ${pl.tracks.length > 4 ? `<p style="font-family:Courier,monospace;font-size:11px;color:#999;margin:8px 0 0;">+ ${pl.tracks.length - 4} more</p>` : ''}
        </div>
        <div style="padding:0 16px 14px;">
          <a href="${pl.spotifyUrl}" style="display:inline-block;font-family:Courier,monospace;font-size:12px;font-weight:700;letter-spacing:0.06em;text-transform:uppercase;color:#FFFDF5;background:#111;padding:10px 20px;border-radius:6px;text-decoration:none;">Listen on Spotify</a>
        </div>
      </div>`;
  }).join('');
...
<body style="margin:0;padding:0;background:#FFFDF5;">
  <div style="max-width:560px;margin:0 auto;padding:32px 16px;font-family:Arial,sans-serif;">
    ...
    <p style="font-family:Courier,monospace;font-size:11px;color:#FF4D00;letter-spacing:0.1em;text-transform:uppercase;margin:6px 0 0;">Your first dig</p>
    <hr style="border:none;border-top:2px solid #111;margin:16px 0 24px;" />

    <h2 style="font-family:Georgia,serif;font-size:28px;font-weight:400;color:#111;margin:0 0 8px;line-height:1.2;">
      ${playlists.length} playlists. ${totalTracks} tracks.<br/>All yours, ${name}.
    </h2>
